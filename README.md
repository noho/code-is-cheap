...
- `claude`, `codex`, `tmux`, `jq`, and `tmux-cli` are available on `PATH`.
...
```zsh
ds_claude() {
  [[ -z "$DEEPSEEK_API_KEY" ]] && echo "DEEPSEEK_API_KEY is not set" && return 1
...
mimo_claude() {
  [[ -z "$MIMO_PLAN_API_KEY" ]] && echo "MIMO_PLAN_API_KEY is not set" && return 1
...
glm_claude() {
  [[ -z "$GLM_API_KEY" ]] && echo "GLM_API_KEY is not set" && return 1
